"""Tests for Q4 text similarity. Face tests rely on the prebuilt index."""
...
def test_text_search_finds_real_estate_top():
    r = search_text("I run a real estate firm and want to capture inbound leads", top_k=1)
...
def test_face_search_self_match_is_perfect():
    ...
    if not p.exists():  # CI without faces seeded
        return
    r = search_face(p, top_k=1)
    if not r.hits:  # face index may not be present
        return
    # querying with face_1 against a corpus that includes face_1 should hit ~1.0
