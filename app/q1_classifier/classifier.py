...
def _mock_classify(lead_json: str) -> dict:
    ...
    has_intent = any(
        (w in msg for w in ("buy", "purchase", "demo", "pricing", "ready"))
    )
    is_cold_signal = any(
        (w in msg for w in ("just curious", "browsing", "later", "exploring"))
    )
    missing: list[str] = []
...
def _mock_respond(lead_json: str, classification_json: str) -> str:
    ...
    cat = cls["category"]
    if cat == "hot":
        return f"Hi {name}, thanks for reaching out — sounds like you're ready to move. Happy to walk you through KeaBuilder live. I have Tuesday 10am or Thursday 2pm open this week — which works? Sam from KeaBuilder"
    if cat == "warm":
        return f"Hi {name}, glad you're looking into KeaBuilder. Most teams in your spot find our funnel templates the fastest way to evaluate fit — happy to send a short walkthrough or answer specific questions. What's most useful? Sam from KeaBuilder"
    return f"Hi {name}, appreciate you stopping by. To point you to the most useful resource, could you share a bit about what you're trying to build — is it a lead-capture funnel, a course, or something else? Sam from KeaBuilder"
